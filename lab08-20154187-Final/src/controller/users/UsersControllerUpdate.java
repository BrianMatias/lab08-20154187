					...
					if(aSearch.isEmpty()){
						...
						if(request.getParameter("action").equals("updating")){
							...
							User aux;
								 ...
								 String query6 = "select from " + User.class.getName();
								 List<User> users = (List<User>)pm.newQuery(query6).execute();
								 for(int i = 0;i<users.size() &&users.get(i) != null;i++){
									 if(users.get(i).getId()== Long.parseLong((request.getParameter("usersId")))){
										 aux = users.get(i);
										 aux.setName(request.getParameter("name"));
										 aux.setMail(request.getParameter("mail"));
										 aux.setGender(request.getParameter("gender"));
										 aux.setIdRole(Long.parseLong((request.getParameter("idRole"))));
									 ...
									 pm.close();
									 response.sendRedirect("/users");//modificar
